        ...
        <div class="savings-grid">
            @foreach ($tabungan as $t)
                <div class="savings-card">
                    ...
                    <div class="pb-3">
                        <form id="delete-form-{{ $t->id }}" action="{{ route('hapus-tabungan', $t->id) }}"
                            method="POST">
                            <div class="savings-buttons-container">
                                <a class="savings-button edit" data-bs-toggle="modal"
                                    data-bs-target="#editTabungan{{ $t->id }}">
                                    <i class="fa fa-edit"></i> Ubah Data
                                </a>

                                @csrf
                                @method('DELETE')
                                <button type="button" class="savings-button hapus"
                        ...
                        @if ($t->nominal_terkumpul < $t->target_nominal)
                            <div class="savings-buttons-container">
                                <button class="savings-button" data-bs-toggle="modal"
                                    data-bs-target="#Menabung{{ $t->id }}">
                            ...
                            <form action="{{ route('edit-tabungan', $t->id) }}" method="POST"
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control" id="target_tanggal"
                                            name="target_tanggal" value="{{ $t->target_tanggal }}" required>
                                    </div>

                                    <div class="mb-3">
                                        <label for="nominal_terkumpul" class="form-label">Nominal Terkumpul</label>
                                        <input type="number" min="100" class="form-control"
                                            id="nominal" name="nominal_terkumpul"
                                            value="{{ $t->nominal_terkumpul }}"
                                            placeholder="Masukkan nominal awal tabungan">
                                        <small class="text-muted"><i>Nominal tidak boleh melebihi target</i></small>
                                    </div>
                                </div>
                            ...
                            <form action="{{ route('menabung', $t->id) }}" method="POST">
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        ...
                                        <input type="number" min="100" class="form-control"
                                            id="nominal_terkumpul" name="nominal"
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        const nominalInput = document.getElementById('nominal{{ $t->id }}');
                        const alertNominal = document.getElementById('alertNominal{{ $t->id }}');

                        // Simulasi data target_nominal dan nominal_terkumpul
                        const targetNominal = {{ $t->target_nominal }};
                        let nominalTerkumpul = {{ $t->nominal_terkumpul }};

                        nominalInput.addEventListener('input', function() {
                            const currentNominal = parseFloat(nominalInput.value) || 0;

                            if ((nominalTerkumpul + currentNominal) > targetNominal) {
                                alertNominal.classList.remove('d-none');
                            } else {
                                alertNominal.classList.add('d-none');
                            }
                        });
                    });
                </script>
